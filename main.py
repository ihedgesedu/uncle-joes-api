...
def get_members():
    ...
    query = f"""
    SELECT id, first_name, last_name, email, phone_number, home_store 
...
def create_member(mem: MemberCreate):
    ...
    query = f"""
        INSERT INTO 
        `{GCP_PROJECT}.{DATASET}.members` (id, first_name, last_name, email, home_store, password)
        VALUES (@id,
         @first_name,
         @last_name,
         @email,
